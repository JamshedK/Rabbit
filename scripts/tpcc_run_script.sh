...
run_with_timing() {
    local seed=$1
    local logfile="logs/tpcc_logs/tpcc_200_run${seed}.log"
    ...
    echo "Starting TPC-C run with seed ${seed} at $(date)" > $logfile
    ...
    python run.py --db postgres --test tpcc --timeout 180 -seed ${seed} >> $logfile 2>&1
    local exit_code=$?
    echo "End time: $(date '+%Y-%m-%d %H:%M:%S')" >> $logfile
    echo "End time (Unix): $(date +%s)" >> $logfile
    echo "Finished TPC-C run with seed ${seed} at $(date). Exit code: $exit_code" >> $logfile
    echo "Completed TPC-C run ${seed} with exit code $exit_code"
    
    # Save results after run completion
    local run_num=$2
    echo "Saving results for run ${run_num}..." >> $logfile
    ./scripts/save_results_reset.sh tpcc ${run_num} >> $logfile 2>&1
...
echo "All TPC-C runs completed! Check logs in logs/tpcc_logs/"