...
class DBMSTemplate(ABC):
    ...
    def recover_dbms(self):
        ...
        logger.info("DBMS recovered")

    def run_command(self,command):
        """运行 shell 命令并返回输出和错误信息"""
        try:
    ...
    def is_mysql_active(self):
        """检查 MySQL 服务是否正在运行"""
        stdout, _ = self.run_command("systemctl is-active mysql")
        return stdout == "active"

    def restart_dbms(self):
        os.system(self.restart_cmd)
        logger.info("DBMS restarted")


    def create_template(self, test):
        self.copy_db(source_db="benchbase", target_db=f"{test}_template")
        logger.info(f"created {test}_template for {test}")
        return True
