...
from transfer.surrogate.base.base_model import AbstractModel
import transfer.surrogate.base.gp_base_prior
...
class BaseGP(AbstractModel):
    ...
    ) -> List[List[transfer.surrogate.base.gp_base_prior.Prior]]:
                    ...
                    if add_bound_priors:
                        if add_soft_bounds:
                            priors_for_hp.append(
                                transfer.surrogate.base.gp_base_prior.SoftTopHatPrior(
                                    lower_bound=bounds[i][0], upper_bound=bounds[i][1], rng=self.rng, exponent=2,
                                ))
                        else:
                            priors_for_hp.append(
                                transfer.surrogate.base.gp_base_prior.TophatPrior(
